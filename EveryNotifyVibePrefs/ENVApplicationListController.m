...
- (void)loadDisabledApps {
    NSArray *fileArray = [NSArray arrayWithContentsOfFile:ENVDisabledAppsFile];
    if ([fileArray isKindOfClass:[NSArray class]]) {
        _disabledApps = [NSMutableSet setWithArray:fileArray];
        return;
    }
...
- (void)saveDisabledApps {
    ...
    // Keep the normal preference value for compatibility.
    CFPreferencesSetAppValue(CFSTR("DisabledApps"), (__bridge CFArrayRef)sorted, ENVPreferencesDomain);
    CFPreferencesAppSynchronize(ENVPreferencesDomain);

    // Also mirror it directly so SpringBoard always sees the latest per-app list.
    [sorted writeToFile:ENVDisabledAppsFile atomically:YES];
